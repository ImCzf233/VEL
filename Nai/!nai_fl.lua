local label1=ui.new_label("AA", "Fake lag","Nai-Fakelag")
local maxcmdticks = ui.reference("MISC", "Settings", "sv_maxusrcmdprocessticks")
local fakeduck = ui.reference("RAGE", "Other", "Duck peek assist")
local infiniteduck = ui.reference("MISC", "Movement", "Infinite duck")
ui.set_visible(maxcmdticks, false)
local fakelag = ui.reference("AA", "Fake lag", "Limit")
local enabled2 = ui.new_checkbox("AA", "Fake lag", "Fakelag Fix")
local enabled = ui.new_checkbox("AA", "Fake lag", "On Shot AA")
local onshotfl = ui.new_slider("AA", "Fake lag", "OnShot-Fakelag", 1, 14, 11 ,true, "t")
local TickDelay = globals.tickcount()
client.set_event_callback("setup_command", function(cmd)
if ui.get(enabled2) then
  if TickDelay < globals.tickcount() then
    cmd.allow_send_packet = true
    TickDelay = globals.tickcount() + ui.get(fakelag) --Choose your fakelag so 1 if you always want 1 fakelag etc
  else
...
client.set_event_callback("weapon_fire",function(e)
   local user_id = client.userid_to_entindex(e.userid)
    if user_id == entity.get_local_player()  then
if ui.get(enabled) then	
    ui.set(fakelag,ui.get(onshotfl))
    TickDelay = globals.tickcount() + ui.get(onshotfl) --Choose your fakelag so 1 if you always want 1 fakelag etc
	client.delay_call(0.01,ui.set,fakelag,13)
	end	
end 
         end)
local shifters = ui.new_multiselect("AA", "Fake lag", "Lag comp shifter", "Default", "Hit ground")
...
client.set_event_callback("setup_command", function (cmd)
    ...
    ui.set(amount, lag_shit[lag_info.current_phase].amount)
    ui.set(variance, lag_shit[lag_info.current_phase].variance)
    ui.set(limit, cmd.in_attack == 0 and lag_shit[lag_info.current_phase].limit or 13)
...
local checkbox_FL_ind = new_checkbox("aa", "Fake lag", "Fake lag indicator by Bacalhauz")
local ui_color = ui.new_color_picker("aa", "Fake lag", "Desync color", "255", "105", "180", "255")
...
local function indicator_paint(ctx)
    ...
    elseif Status == "PEEKING" then
        ind_S_r, ind_S_g, ind_S_b = 246, 127, 117 -- -- vermelho   
    ...
    client.draw_gradient(ctx, get_x + 5, get_y+d_h, w, 16, 0, 0, 0, 255, 10, 10, 10, 30, true)    -- bottom bar text
...
--128, 183, 255
    ...
    renderer.text(get_x + 158,get_y + 2, 237, 137, 146, 217, '', 0, "Nai")
