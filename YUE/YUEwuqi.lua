...
local name_to_num = { ["Global"] = 1, ["Taser"] = 2, ["Heavy Pistol"] = 3, ["Pistol"] = 4, ["Auto"] = 5, ["Scout"] = 6, ["AWP"] = 7, ["Rifle"] = 8, ["SMG"] = 9, ["Shotgun"] = 10 }
...
local master_switch = ui_new_checkbox("LUA", "A", "自適應武器")
local active_wpn = ui_new_combobox("LUA", "A", "武器選擇", config_names)
local rage = {}
local active_idx = 1
...
for i=1, #config_names do
    rage[i] = {
        ...
        target_selection = ui_new_combobox("LUA", "A", "[" .. config_names[i] .. "] 目標選擇", {"Cycle", "Cycle (2x)", "Near crosshair", "Highest damage", "Lowest ping", "Best K/D ratio", "Best hit chance"}),
        accuracy_boost = ui_new_combobox("LUA", "A", "[" .. config_names[i] .. "] 精准提升", {"Off", "Low", "Medium", "High", "Maximum"}),
        target_hitbox = ui_new_multiselect("LUA", "A", "[" .. config_names[i] .. "] 部位選擇", { "Head", "Chest", "Arms", "Stomach", "Legs", "Feet" }),
        multipoint = ui_new_multiselect("LUA", "A", "[" .. config_names[i] .. "] 多點選擇", { "Head", "Chest", "Arms", "Stomach", "Legs", "Feet" }), 
        multimode = ui_new_combobox("LUA", "A", "\n[" .. config_names[i] .. "] 多點分佈", { "Low", "Medium", "High" }),
        multipoint_scale = ui_new_slider("LUA", "A", "[" .. config_names[i] .. "] 多點邊緣", 24, 100, 60, true, "%", 1, { [24] = "Auto" }),
        dt_mp_enable = ui_new_checkbox("LUA", "A", "[" .. config_names[i] .. "] DT 多點開啟"),
        dt_multipoint = ui_new_multiselect("LUA", "A", "[" .. config_names[i] .. "] DT 多點", { "Head", "Chest", "Arms", "Stomach", "Legs", "Feet" }),   
        dt_multimode = ui_new_combobox("LUA", "A", "\n[" .. config_names[i] .. "] DT 多點分佈", { "Low", "Medium", "High" }),   
        dt_multipoint_scale = ui_new_slider("LUA", "A", "[" .. config_names[i] .. "] DT 多點邊緣", 24, 100, 60, true, "%", 1, { [24] = "Auto" }),
        prefer_safe_point = ui_new_checkbox("LUA", "A", "[" .. config_names[i] .. "] 更喜歡安全點"),
        dt_prefer_safe_point = ui_new_checkbox("LUA", "A", "[" .. config_names[i] .. "] DT更喜歡安全點"),
        force_safe_point_onlimbs = ui_new_checkbox("LUA", "A", "[" .. config_names[i] .. "] 強制四肢安全點"),
        automatic_fire = ui_new_checkbox("LUA", "A", "[" .. config_names[i] .. "] 自動開火"),
        automatic_penetration = ui_new_checkbox("LUA", "A", "[" .. config_names[i] .. "] 自動穿墻"),
        automatic_scope = ui_new_checkbox("LUA", "A", "[" .. config_names[i] .. "] 自動開鏡"),
        silent_aim = ui_new_checkbox("LUA", "A", "[" .. config_names[i] .. "] 靜默"),
        hitchance = ui_new_slider("LUA", "A", "[" .. config_names[i] .. "] 普通命中率", 0, 100, 50, true, "%", 1, {"Off"}),
        air_hc_enable = ui_new_checkbox("LUA", "A", "[" .. config_names[i] .. "] 開啟在空中命中率"),
        hitchance_air = ui_new_slider("LUA", "A", "[" .. config_names[i] .. "] 空中命中率", 0, 100, 50, true, "%", 1, {"Off"}),
        custom_damage = ui_new_multiselect("LUA", "A", "[" .. config_names[i] .. "] 自定義最小傷害", { "Visible", "Double tap", "On-key" }),
        min_damage = ui_new_slider("LUA", "A", "[" .. config_names[i] .. "] 最小傷害", 0, 126, 20, true, nil, 1, damage_idx),
        vis_min_damage = ui_new_slider("LUA", "A", "[" .. config_names[i] .. "] 可見最小傷害", 0, 126, 20, true, nil, 1, damage_idx),
        wall_min_damage = ui_new_slider("LUA", "A", "[" .. config_names[i] .. "] 穿墻最小傷害", 0, 126, 20, true, nil, 1, damage_idx),
        dt_min_damage = ui_new_slider("LUA", "A", "[" .. config_names[i] .. "] DT 最小傷害", 0, 126, 20, true, nil, 1, damage_idx),
        ovr_min_damage = ui_new_slider("LUA", "A", "[" .. config_names[i] .. "] 強制按鍵最小傷害", 0, 126, 20, true, nil, 1, damage_idx),
        ovr_key = ui_new_hotkey("LUA", "A", "[" .. config_names[i] .. "] 強制按鍵最小傷害", true),
        quick_stop = ui_new_checkbox("LUA", "A", "[" .. config_names[i] .. "] 快速急停"),
        quick_stop_options = ui_new_multiselect("LUA", "A", "[" .. config_names[i] .. "] 快速急停選擇", {"Early", "Slow motion", "Duck", "Move between shots", "Ignore molotov"}),
        prefer_baim = ui_new_checkbox("LUA", "A", "[" .. config_names[i] .. "] 更喜歡打身體"),
        prefer_baim_disablers = ui_new_multiselect("LUA", "A", "[" .. config_names[i] .. "] 在X情況下關閉更喜歡打身體", {"Low inaccuracy", "Target shot fired", "Target resolved", "Safe point headshot", "Low damage", "High pitch", "Sideways"}),
        delay_shot = ui_new_checkbox("LUA", "A", "[" .. config_names[i] .. "] 延遲開槍"),
        force_baim_peek = ui_new_checkbox("LUA", "A", "[" .. config_names[i] .. "] 在對方Peek啟用打身体")
    }
end

local prioritize_awp = ui_new_checkbox("LUA", "A", "優先攻擊AWP使用者")
...
local ref_force_safepoint_onlimbs = ui_reference("RAGE", "Aimbot", "Force safe point on limbs")
...
local function run_visuals()
    ...
    if ui_get(rage[active_idx].ovr_key) and entity_is_alive(plocal) then
        renderer_text(cx + 3, cy + 3, 255, 255, 255, 255, "b", 0, tostring(ui_get(ref_mindamage)))
    end
end
...
local function handle_menu()
    ...
    for i=1, #config_names do
        ...
        ui_set_visible(rage[i].ovr_key, show and contains(ui_get(rage[i].custom_damage), "On-key"))
...
local function set_config(idx)
    ...
    if custom_damage then
        if not ui_get(rage[i].ovr_key) or not contains(ui_get(rage[i].custom_damage), "On-key") then
    ...
    ui_set(ref_target_hitbox, ui_get(rage[i].target_hitbox))
    ...
    ui_set(ref_force_safepoint_onlimbs, ui_get(rage[i].force_safe_point_onlimbs))
